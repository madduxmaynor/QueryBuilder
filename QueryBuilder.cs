...
namespace QueryBuilderStarter
{
    public class QueryBuilder : IDisposable
    {
        ...
        public void Create<T>(T item) where T : IClassModel
        {
            PropertyInfo[] properties = typeof(T).GetProperties();
            var names = new List<string>();
            var values = new List<string>();
            ...
            string tableName = typeof(T).Name;
            string insertColumns = string.Join(", ", names);
            string insertValues = string.Join(", ", values);

            var command = connection.CreateCommand();
            command.CommandText = $"INSERT INTO {tableName} ({insertColumns}) VALUES ({insertValues})";

            for (int i = 1; i < properties.Length; i++)
            {
                PropertyInfo property = properties[i];
                command.Parameters.AddWithValue($"@param{i}", property.GetValue(item));
            }
